...
import { Sparkles, Star, Copy, RefreshCcw, ThumbsUp, ThumbsDown, Clock, MessageSquare, Loader2, CheckCircle, MapPin, BarChart3, TrendingUp, AlertTriangle, ChevronRight, PieChart } from "lucide-react";
        ...
        <div className="space-y-6 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-lg shadow-gray-100/50 p-6 space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 text-sm">Google Maps Reviews Link</h3>
                <p className="text-xs text-gray-500">Provide the URL to study customer reviews & generate reply suggestions</p>
              </div>
            </div>

            <div className="grid md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <label className="text-sm font-semibold text-gray-700 mb-2 block">Google Maps Reviews URL</label>
                <input
                  type="text"
                  value={mapsUrl}
                  onChange={(e) => setMapsUrl(e.target.value)}
                  placeholder="Paste link (e.g., https://maps.app.goo.gl/... or https://google.com/maps/...)"
                  className="w-full h-11 rounded-xl border-2 border-gray-200 bg-white px-3 text-sm text-gray-700 focus:border-[#8B5CF6] focus:ring-2 focus:ring-[#8B5CF6]/20 outline-none transition-all placeholder:text-gray-400"
                />
              </div>

              <div>
                <label className="text-sm font-semibold text-gray-700 mb-2 block">Reply Tone</label>
                <select
                  value={mapsTone}
                  onChange={(e) => setMapsTone(e.target.value)}
                  className="w-full h-11 rounded-xl border-2 border-gray-200 bg-white px-3 text-sm text-gray-700 focus:border-[#8B5CF6] focus:ring-2 focus:ring-[#8B5CF6]/20 outline-none transition-all"
                >
                  {tones.map((t) => (
            ...
            {mapsError && (
              <div className="flex items-start gap-3 p-3 rounded-xl bg-red-50 border border-red-200">
                ...
                <span className="text-red-600 text-xs font-medium">{mapsError}</span>
              </div>
            )}
          ...
          {mapsAnalysisResult && (
            <div className="space-y-6">
              <div className="flex items-center gap-3 p-4 bg-[#F3EEFF]/40 rounded-2xl border border-[#E9D5FF]/60">
                <div className="p-2 bg-[#8B5CF6] text-white rounded-xl shadow-md">
                  <MapPin size={18} />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-gray-900">{mapsAnalysisResult.business_name}</h2>
                  <p className="text-xs text-[#8B5CF6] font-medium">Google Maps Reviews & AI Reply Analysis Dashboard</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* 1. Rating Summary */}
                <div className="bg-white rounded-2xl border border-gray-200/60 p-5 space-y-3 shadow-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Rating Summary</span>
                    ...
                    <div className="flex items-center gap-1 mt-1.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star 
                          key={s} 
                          size={13} 
                          className={s <= Math.round(mapsAnalysisResult.analysis.average_rating) ? "text-yellow-400" : "text-gray-200"}
                          fill={s <= Math.round(mapsAnalysisResult.analysis.average_rating) ? "currentColor" : "none"}
                        />
                      ))}
                      <span className="text-xs text-gray-400 ml-1">({mapsAnalysisResult.analysis.total_reviews_count} reviews)</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200/60 p-5 space-y-3 shadow-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Sentiment Breakdown</span>
                    <PieChart size={18} className="text-[#8B5CF6]" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-bold">
                      <span className="text-emerald-600">Pos: {mapsAnalysisResult.analysis.sentiment_breakdown.positive_percentage}%</span>
                      <span className="text-amber-500">Neu: {mapsAnalysisResult.analysis.sentiment_breakdown.neutral_percentage}%</span>
                      <span className="text-red-500">Neg: {mapsAnalysisResult.analysis.sentiment_breakdown.negative_percentage}%</span>
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200/60 p-5 space-y-2 md:col-span-2 shadow-sm">
                  <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block">AI Sentiment Analysis</span>
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-200/60 p-6 space-y-4 shadow-sm">
                  <div className="flex items-center gap-2 pb-3 border-b border-gray-100">
                    <TrendingUp size={16} className="text-[#8B5CF6]" />
                    <h3 className="font-semibold text-gray-900 text-sm">Actionable Suggestions & Priorities</h3>
                  </div>
                    ...
                    {mapsAnalysisResult.analysis.actionable_suggestions.map((s, idx) => (
                      ...
                      >
                        <div className="space-y-1">
                          <p className="text-xs font-bold text-gray-800">{s.suggestion}</p>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-gray-400 font-bold uppercase">{s.category}</span>
                            <span className="text-gray-300">•</span>
                            <span className="text-[10px] text-gray-500 font-semibold">{s.frequency_percentage}% mention rate</span>
                          </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200/60 p-6 space-y-4 shadow-sm">
                  <div className="flex items-center gap-2 pb-3 border-b border-gray-100">
                    ...
                    <h3 className="font-semibold text-gray-900 text-sm">Customer Mention Counts</h3>
                  </div>
                  <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-200/60 p-6 space-y-5 shadow-sm">
                <div className="flex items-center gap-2 pb-4 border-b border-gray-100">
                  <MessageSquare size={16} className="text-[#8B5CF6]" />
                  <h3 className="font-semibold text-gray-900 text-sm">Generated Customer Replies</h3>
                </div>
                <div className="space-y-4">
                  {mapsAnalysisResult.reviews.map((r, idx) => (
                    <div key={idx} className="p-4 rounded-xl border border-gray-100 bg-gray-50/40 space-y-3 hover:border-[#8B5CF6]/25 transition-all">
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <p className="text-xs font-bold text-gray-800">{r.reviewer_name}</p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-yellow-500">{"★".repeat(r.rating)}</span>
                            <span className="text-gray-300">•</span>
                            <span className="text-[10px] text-gray-400">{r.rating}/5 rating</span>
                          </div>
                        </div>
                      </div>
                      <p className="text-xs text-gray-600 italic">"{r.comment}"</p>
                      
                      <div className="p-3.5 rounded-xl bg-white border border-gray-100 shadow-sm space-y-2">
                        <div className="flex justify-between items-center pb-2 border-b border-gray-50">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-[#8B5CF6] transition-colors"
                          >
                        ...
                        <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-line">{r.reply}</p>
                      </div>
                    </div>
                  ))}
                </div>
