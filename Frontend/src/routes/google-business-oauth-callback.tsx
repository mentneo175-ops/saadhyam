...
export const Route = createFileRoute("/google-business-oauth-callback")({
  component: GoogleBusinessOAuthCallback,
});

function GoogleBusinessOAuthCallback() {
  useEffect(() => {
    ...
    if (error) {
      if (window.opener) {
        window.opener.postMessage(
          {
            type: "google-business-oauth-error",
    ...
    if (code) {
      if (window.opener) {
        window.opener.postMessage(
          {
            type: "google-business-oauth-success",
            data: {
              code: code,
              state: state,
            },
